...
class Plotter(object):
    ...
    def __init__(self, analysis, base_selections, ntuple_dir, out_dir, channels=[], lumi=19.7):
    ...
    def add_group(self, group_name, label, *sample_names, **kwargs):
        ...
        self.sample_groups[group_name] = {
                'sample_names': [os.path.splitext(os.path.basename(x))[0] for x in samples],
                'edgecolor': edgecolor,
                'facecolor': facecolor,
                'label': label}
...
def main():
    plotter = Plotter("DblH4l", "(mass > 80)", "./ntuples", "./plots/test", channels=["mmmm"], lumi=19.7)

    plotter.add_group("hpp", "HPP", "HPlus*110*", facecolor='mediumorchid', edgecolor='indigo')
    plotter.add_group("zz", "ZZ", "ZZTo*", facecolor='lightskyblue', edgecolor='darkblue')
